...
public:
    ...
    * @brief 描画する。
    */
    void Render(bool updateModels = true);
...
private:
    ...
    bool IsSwitchButtonHit(float pointX, float pointY) const;
    void UpdateSwitchButtonRect(int width, int height);

    TouchManager_Common* _touchManager;                 ///< タッチマネージャー
    LAppSprite* _back;                       ///< 背景画像
    LAppSprite* _gear;                       ///< ギア画像
    LAppSprite* _power;                      ///< 電源画像
    HitAreaRect _switchButtonRect;           ///< 無描画の切り替えボタン当たり判定

    // レンダリング先を別ターゲットにする方式の場合に使用
    LAppSprite* _renderSprite;                                  ///< モードによっては_renderBufferのテクスチャを描画
    Csm::Rendering::CubismRenderTarget_PortableGL _renderBuffer;   ///< モードによってはCubismモデル結果をこっちにレンダリング
