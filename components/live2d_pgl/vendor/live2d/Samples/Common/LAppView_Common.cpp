...
void LAppView_Common::ApplyDebugViewTransform(float scaleFactor, float translateX, float translateY)
{
    if (_viewMatrix == NULL)
    {
        return;
    ...
    {
        _viewMatrix->ScaleRelative(scaleFactor, scaleFactor);
    }

    if (translateX != 0.0f || translateY != 0.0f)
    {
        _viewMatrix->TranslateRelative(translateX, translateY);
    }
}
