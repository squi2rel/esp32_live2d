...
void CubismModel::Initialize()
{
    ...
    {
        const csmChar* const* parameterIds = Core::csmGetParameterIds(_model);
    ...
    // Part
    {
        const csmChar* const* partIds = Core::csmGetPartIds(_model);
    ...
    // Drawable
    {
        const csmChar* const* drawableIds = Core::csmGetDrawableIds(_model);
...
CubismIdHandle CubismModel::GetOffscreenOwnerId(csmUint32 offscreenIndex) const
{
...
{
    const Core::csmUint16* const* indicesArray = Core::csmGetDrawableIndices(_model);
    return reinterpret_cast<const csmUint16*>(indicesArray[drawableIndex]);
}

const Core::csmVector2* CubismModel::GetDrawableVertexPositions(csmInt32 drawableIndex) const
{
    const Core::csmVector2* const* verticesArray = Core::csmGetDrawableVertexPositions(_model);
    return verticesArray[drawableIndex];
}

const Core::csmVector2* CubismModel::GetDrawableVertexUvs(csmInt32 drawableIndex) const
{
    const Core::csmVector2* const* uvsArray = Core::csmGetDrawableVertexUvs(_model);
...
{
    const csmInt32* const* masks = Core::csmGetDrawableMasks(_model);
    return const_cast<const csmInt32**>(masks);
}
...
{
    const csmInt32* const* masks = Core::csmGetOffscreenMasks(_model);
    return const_cast<const csmInt32**>(masks);
}
